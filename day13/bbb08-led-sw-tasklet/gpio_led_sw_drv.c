...
// helper functions
//  in_interrupt() - check if execution is in interrupt context
//  in_irq() - check if execution is in isr
...
// switch tasklet handler declaration
void my_switch_tasklet_handler(unsigned long param);
// tasklet struct
...
static int __init pchar_init(void) {
    ...
    // switch tasklet init
    tasklet_init(&switch_tasklet, my_switch_tasklet_handler, 0);
    pr_info("%s: tasklet_init() initialized tasklet handler for switch pin %d.\n", THIS_MODULE->name, GPIO_SWITCH);
...
// switch tasklet handler implementation
void my_switch_tasklet_handler(unsigned long param) {
    pr_info("%s: my_switch_tasklet_handler() called: in interrupt context=%s.\n", THIS_MODULE->name, (in_interrupt()?"yes":"no"));
    // toggle the led
    led_state = !led_state;
    gpio_set_value(GPIO_LED, led_state);
}
