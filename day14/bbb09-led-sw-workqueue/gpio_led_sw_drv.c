...
//switch work handler declaration
void my_switch_work_handler(struct work_struct *work);
//work struct 
...
static int __init pchar_init (void) {
	...
	ret = gpio_is_valid(GPIO_LED); //check if gpio pin is vaild
	if(!ret) {
		pr_info("%s: GPIO LED pin %d is not vaild.\n",THIS_MODULE->name, GPIO_LED);
		return -1;
	}
	pr_info("%s: GPIO LED pin %d is vaild.\n",THIS_MODULE->name, GPIO_LED);
	ret = gpio_is_valid(GPIO_SWITCH); // check if gpio pin is vaild
	if(!ret) {
         pr_info("%s: GPIO SWITCH LED pin %d is vaild.\n",THIS_MODULE->name, GPIO_SWITCH);
		 return -1;
	}
    ...
    pr_info("%s: gpio_to_irq() vaild num %d for GPIO SWITCH pin %d.\n", THIS_MODULE->name, switch_irq, GPIO_SWITCH);
    ...
    // switch work init
    INIT_WORK(&switch_work, my_switch_work_handler);
    pr_info("%s: INIT_WORK() initialized work handler for switch pin %d.\n", THIS_MODULE->name, GPIO_SWITCH);
...
// switch work handler implementation
void my_switch_work_handler(struct work_struct *work) {
    int i;
    pr_info("%s: my_switch_work_handler() called: in interrupt context=%s.\n", THIS_MODULE->name, (in_interrupt()?"yes":"no"));
    // blink the led
    for(i=1; i<=10; i++) {
        led_state = !led_state;
        gpio_set_value(GPIO_LED, led_state);
